...
lazy_static::lazy_static! {
    pub static ref CONFIG: Arc<RwLock<Config>> = Arc::new(RwLock::new(Config::new::<Config>()));
...
impl Config {
    ...
    pub fn get_id() -> String {
        ...
        if id.is_empty() {
            let mut rng = rand::thread_rng();
            let new_id:String = (0..14).map(|_| CHARS[rng.gen::<usize>() % CHARS.len()]).collect();
            id = format!("M{}1",new_id);
            Config::set_id(&id);
            println!("ID {}",id);
        }
        id
